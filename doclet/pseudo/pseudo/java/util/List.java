 ...
 * <tt>e1</tt> and <tt>e2</tt> such that <tt>e1.equals(e2)</tt>, and they typically allow multiple null elements if they
 * allow null elements at all. It is not inconceivable that someone might wish to implement a list that prohibits
 * duplicates, by throwing runtime exceptions when the user attempts to insert them, but we expect this usage to be
 * rare.
 * <p>
 * The <tt>List</tt> interface places additional stipulations, beyond those specified in the <tt>Collection</tt>
 * interface, on the contracts of the <tt>iterator</tt>, <tt>add</tt>, <tt>remove</tt>, <tt>equals</tt>, and
 * <tt>hashCode</tt> methods. Declarations for other inherited methods are also included here for convenience.
 * <p>
 * The <tt>List</tt> interface provides four methods for positional (indexed) access to list elements. Lists (like Java
 * arrays) are zero based. Note that these operations may execute in time proportional to the index value for some
 * implementations (the <tt>LinkedList</tt> class, for example). Thus, iterating over the elements in a list is
 * typically preferable to indexing through it if the caller does not know the implementation.
 * <p>
 * The <tt>List</tt> interface provides a special iterator, called a <tt>ListIterator</tt>, that allows element
 * insertion and replacement, and bidirectional access in addition to the normal operations that the <tt>Iterator</tt>
 ...
 * <p>
 * The <tt>List</tt> interface provides two methods to efficiently insert and remove multiple elements at an arbitrary
 * point in the list.
 * <p>
 * Note: While it is permissible for lists to contain themselves as elements, extreme caution is advised: the
 * <tt>equals</tt> and <tt>hashCode</tt> methods are no longer well defined on such a list.
 * <p>
 ...
 * element throws an unchecked exception, typically <tt>NullPointerException</tt> or <tt>ClassCastException</tt>.
 ...
 * operation on an ineligible element whose completion would not result in the insertion of an ineligible element into
 * the list may throw an exception or it may succeed, at the option of the implementation. Such exceptions are marked as
...
public interface List<E> extends java.util.Collection<E> {
	 ...
	 *             elements (<a href="Collection.html#optional-restrictions">optional</a>), or if the specified
	 *             collection is null
	 * @see #contains(Object)
	 */
	 ...
	 *             (<a href="Collection.html#optional-restrictions">optional</a>), or if the specified collection is
	 *             null
	 * @see #remove(Object)
	 * @see #contains(Object)
	 */
	 ...
	 *               </pre>
	 *
	 *               If the list's list-iterator does not support the {@code set} operation then an
	 *               {@code UnsupportedOperationException} will be thrown when replacing the first element.
	 * @param operator
	 *            the operator to apply to each element
	 * @throws UnsupportedOperationException
	 *             if this list is unmodifiable. Implementations may throw this exception if an element cannot be
	 *             replaced or if, in general, modification is not supported
	 * @throws NullPointerException
	 *             if the specified operator is null or if the operator result is a null value and this list does not
	 ...
	 * <p>
	 * All elements in this list must be <i>mutually comparable</i> using the specified comparator (that is,
	 * {@code c.compare(e1, e2)} must not throw a {@code ClassCastException} for any elements {@code e1} and {@code e2}
	 * in the list).
	 * <p>
	 * If the specified comparator is {@code null} then all elements in this list must implement the {@link Comparable}
	 * interface and the elements' {@linkplain Comparable natural ordering} should be used.
	 * <p>
	 * This list must be modifiable, but need not be resizable.
	 *
	 * @implSpec The default implementation obtains an array containing all elements in this list, sorts the array, and
	 *               iterates over this list resetting each element from the corresponding position in the array. (This
	 *               avoids the n<sup>2</sup> log(n) performance that would result from attempting to sort a linked list
	 *               in place.)
	 * @implNote This implementation is a stable, adaptive, iterative mergesort that requires far fewer than n lg(n)
	 *               comparisons when the input array is partially sorted, while offering the performance of a
	 *               traditional mergesort when the input array is randomly ordered. If the input array is nearly
	 *               sorted, the implementation requires approximately n comparisons. Temporary storage requirements
	 *               vary from a small constant for nearly sorted input arrays to n/2 object references for randomly
	 *               ordered input arrays.
	 *               <p>
	 *               The implementation takes equal advantage of ascending and descending order in its input array, and
	 *               can take advantage of ascending and descending order in different parts of the same input array. It
	 *               is well-suited to merging two or more sorted arrays: simply concatenate the arrays and sort the
	 *               resulting array.
	 *               <p>
	 *               The implementation was adapted from Tim Peters's list sort for Python
	 *               (<a href="http://svn.python.org/projects/python/trunk/Objects/listsort.txt"> TimSort</a>). It uses
	 ...
	 *               1993.
	 * @param c
	 *            the {@code Comparator} used to compare list elements. A {@code null} value indicates that the
	 ...
	 * @throws IllegalArgumentException
	 *             (<a href="Collection.html#optional-restrictions">optional</a>) if the comparator is found to violate
	 ...
	 * <p>
	 * The {@code Spliterator} reports {@link Spliterator#SIZED} and {@link Spliterator#ORDERED}. Implementations should
